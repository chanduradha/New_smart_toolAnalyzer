...
//       <div className="bg-white p-4 rounded-md"  style={{ width: '1000px' }} onClick={stopPropagation}>
...
const PolarPlot = ({ isVisible, onClose, fileData }) => {
  ...
  const [initialDateWindow, setInitialDateWindow] = useState([0, 0]);
    ...
    if (isVisible && fileData && typeof fileData === 'string') {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (line.startsWith('Tension;Torsion;Bending moment X;Bending moment Y;Time;Temperature')) {
          for (let j = i + 1; j < rows.length; j++) {
            ...
            if (rowData.length === 6) {
              const xValue = rowData[2];
              const yValue = rowData[3];
      ...
      if (data.length > 0) {
        ...
        polarPlot.ready(() => {
          ...
          if (yLabelElem) {
            yLabelElem.style.transform = 'rotate(-90deg)';
            yLabelElem.style.width = '150px';
            yLabelElem.style.overflow = 'hidden';
            yLabelElem.style.textOverflow = 'ellipsis';
            yLabelElem.style.fontWeight = 'bold';
            yLabelElem.style.fontSize = '16px';
            yLabelElem.style.marginLeft = '150px';
          }
        });
  ...
  const updateGraph = () => {
    ...
    const toastId = toast.loading(`Updating graph with ${displayPoints} points...`);

    // Use requestAnimationFrame to update the graph in the next frame
    requestAnimationFrame(() => {
      ...
      if (displayData.length > 0) {
        const firstPointTime = findPointTime(displayData[0], 0);
        const lastPointTime = findPointTime(displayData[displayData.length - 1], displayData.length - 1);

        updatePointTimes(firstPointTime, lastPointTime);
      }

      // Dismiss the loading toast and show a success message
      toast.dismiss(toastId);
      toast.success(`Graph updated with ${displayPoints} points`);
    });
  };

  
  const findPointTime = useMemo(() => (point, index) => {
    const [x, y] = point;
    for (let i = 0; i < dataPoints.length; i++) {
      if (dataPoints[i][0] === x && dataPoints[i][1] === y) {
        const rawTime = fileData.split('\n')[8 + i].split(';')[4];
  ...
  const updatePointTimes = (firstPointTime, lastPointTime) => {
    const redPointTime = document.getElementById('redPointTime');
    const blackPointTime = document.getElementById('blackPointTime');
  ...
  const updateGraphWithSlider = (value) => {
    if (graph) {
      const numPoints = dataPoints.length;
      const index = Math.floor((value / 100) * (numPoints - 1));

      graph.updateOptions({
        dateWindow: initialDateWindow,
      });

      const rawTime = fileData.split('\n')[8 + index].split(';')[4];
      const pointTime = formatTime(rawTime);
      setSliderValue(value);
      setSliderTime(pointTime);
    }
  };

  useEffect(() => {
    if (graph) {
      setInitialDateWindow(graph.xAxisRange());
    }
  }, [graph]);
